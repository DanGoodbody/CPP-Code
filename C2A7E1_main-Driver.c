...
{
   char *string;                       /* string for this node */
...
void BuildList(BIN *bp, const char *str)
{
   ...
   for (; pNode != NULL; pNode = pNode->next)   /* visit each node */
      if (!strcmp(pNode->string, str))          /* if strings match... */
         break;                                 /* ...quit looking */
   if (pNode == NULL)                           /* no matching node */
   {
      ...
      pNode->string = (char *)SafeMalloc(length);   /* storage for string */
      memcpy(pNode->string, str, length);       /* store string */
